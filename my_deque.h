...
#define _DEQUESIZ	64 // (512 / sizeof (value_type))

//mess((double)sizeof(value_type), "size of value type");
	...
	void _Assign_rv(_Myt&& _Right)
		...
		if (this == &_Right)
			...
			for (iterator _Next = _Right.begin(); _Next != _Right.end();
				++_Next)
				push_back(_STD forward<value_type>(*_Next));
			}
		else
...
_VARIADIC_EXPAND_0X(_DEQUE_EMPLACE, , , , )
#undef _DEQUE_EMPLACE_CHECK
#undef _DEQUE_EMPLACE
	...
	void resize(size_type _Newsize)
		...
		if (_Newsize < this->_Mysize)
			...
			{	// append default-constructed elements
			...
			while (this->_Mysize < _Newsize)
				...
				_Uninitialized_default_fill_n(
					this->_Map[_Block] + _Newoff % _DEQUESIZ, 1, _Alval);
				_PUSH_BACK_END;
				}
			}
		}
